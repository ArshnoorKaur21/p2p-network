...
//this node will create transaction
...
//mining the transactions now and adding the new block and update new difficulty
...
//to one we send messages where we are effectivly passing objects to json, message of the object in this case,their transactions,blocks ,blockchains would dissappear so to solve tbhs rpblm we use static
...
//2. //now we can send transaction like this
// sendMessage(produceMessage('TYPE_CREATE_TRANSACTION',someTransaction))
// JeChain.addTransaction(someTransaction)//adding transaction to pool

//3. mining and sending new blocks
/*
message for suggesting new block
type:"TYPE_REPLACE_CHAIN" we here are not replacing whole chain we are just adding new block
data:[
    //sending the new block and new diffciutly
    'new block',
    'new diffciutly'

    1st we will chq whetehr block is valid or not then we will add to chain and update the diffculty,
    the block is valid when it has valid transactions which means transactions are in tx pool and valid transactions,
    it has valid hash(it matches with block information called block header) , valid diificulty (which means it can be greater or lesser than old difficulty ) and it has valid timesteamp which
    must not be greater than time they sent us and can be less than prev block timestamp
]
*/

/*
for new nodes that hve joined the network they are 2 ways to get latest chain
either get chain from trustworthy source oru can get chain in network but the size of mesaage is limited we cant be able to send whole chain but we can send blocks one by onw
and we will also send information
*/
